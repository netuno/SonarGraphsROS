...
void Sonar::floodFill(unsigned row, unsigned col)
{
    ...
    /*
      Row 1 - Pixel Row
      Row 2 - Pixel Col
      Row 3 - Pixel Intensity
    */
    floodFillResult.at<ushort>(0,floodFillResultCount) = row;
    floodFillResult.at<ushort>(1,floodFillResultCount) = col;
    floodFillResult.at<ushort>(2,floodFillResultCount) = img16bits.at<ushort>(row,col);
...
void Sonar::startFloodFill(unsigned row, unsigned col)
{
    // Clear old floodfill results
    if(floodFillResult.rows != 1)
    {
        cout << "Realoc!!" << endl;
        floodFillResult = Mat(3, maxSampleSize , CV_16UC1, Scalar(0));
    }
...
void Sonar::drawResultPixels(Mat &img, Scalar color)
{
    ...
    {
        // Here:
        //   floodFillResult.at<ushort>(0, k ) - Acess the row of K-th pixel found by floodfill search
        //   floodFillResult.at<ushort>(1, k ) - Acess the col of K-th pixel found by floodfill search
        //   floodFillResult.at<ushort>(2, k ) - Acess the pixel intensity of K-th pixel found by floodfill search
        /// @todo - Acessing uchar Mat::data[] is faster than at<type>()

        unsigned pixelRow = floodFillResult.at<ushort>(0,i),
                pixelCol = floodFillResult.at<ushort>(1,i),
...
void Sonar::rotateResult(unsigned N, float angDegree, float ox, float oy)
{
    ...
    {
        float ny = floodFillResult.at<ushort>(0,i)-oy, // Acess pixel's row
              nx = floodFillResult.at<ushort>(1,i)-ox; // Acess pixel's col

        floodFillResult.at<ushort>(0,i) = // Access the row
                roundf(  // It's important! round the values!!
                         nx*sinA + ny*cosA + oy  // Row <-> y
                      );

        floodFillResult.at<ushort>(1,i) = // Access the col
                roundf(  // It's important! round the values!!
    ...
    for(unsigned row = 0 ; row < img.rows ; row+=linJump)
    {
        for(unsigned col = 0 ; col < img.cols ; col+=colJump)
        {
               ...
               img16bits.at<ushort>(row,col) >= pixelThreshold)
            {
                // Search the sample
                startFloodFill(row,col);
//                Mat sample = floodFillResult(Rect(0,0,floodFillResultCount,3));
                Mat sample = floodFillResult(Rect(0,0,floodFillResultCount,2));
...
//                ang = 180.0*atan2(eVect.at<float>(1,0),eVect.at<float>(1,1))/M_PI;
                ...
                cout << "N:" << endl << floodFillResultCount << endl;

                if(drawPixelFound)
                    drawResultPixels(colorImg,Scalar(255,0,0));
...
/*
                rowStdDev = eVal.at<float>(1,0);
                colStdDev = eVal.at<float>(2,0);
                pixelStdDev = eVal.at<float>(0,0);
*/
                rowStdDev = eVal.at<float>(0,0);
                colStdDev = eVal.at<float>(1,0);
                pixelStdDev = 1.f;

                cout << "rowStdDev: " << rowStdDev << endl;
                cout << "coltdDev: " << colStdDev << endl;
                cout << "pixelStdDev: " << pixelStdDev << endl << endl;

                if(floodFillResultCount >= minSampleSize)
                    sd->gaussians.push_back(Gaussian(colMean,rowMean,pixelMean,
                                             colStdDev*stdDevMultiply,rowStdDev*stdDevMultiply,pixelStdDev,ang,floodFillResultCount));
    ...
    for(unsigned i = 0 ; i < sd->gaussians.size() ; i++)
    {
        ...
        for(unsigned j = i+1 ; j < sd->gaussians.size() ; j++)
        {
            ...
            if(d <= graphLinkDistance)
            {
                ...
                sd->graph[i].push_back(new GraphLink(dt-sd->gaussians[i].ang,d,j));

                if(dt > 180.f) dt-=180.f;
                else dt+= 180.f;

                sd->graph[j].push_back(new GraphLink(dt-sd->gaussians[j].ang,d,i));
            }
        }

...
void Sonar::floodFill(unsigned row, unsigned col, int k)
{
    ...
    */
    ...
    for(unsigned neighbor = 0 ; neighbor < 8 ; neighbor++)
    {
        ...
        if(searchMask.at<uchar>(nextRow,nextCol) != 255)
        {
            ...
            {
                floodFill(nextRow , nextCol, searchMergeDistance);
            }else if(k > 0)
            {
                floodFill(nextRow , nextCol,k-1);
            }
        }
    }
}
                                          ...
                                          float &ang, unsigned &N)
{
    ...
    // Search pixels
    floodFill(row , col, searchMergeDistance);
...
//        matcher.matche(descriptors[descriptors.size()-1], sd,matches);
        matcher.matcheSIFTCut(descriptors[descriptors.size()-1], sd,matches);
